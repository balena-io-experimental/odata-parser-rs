...
//*               "://" host [ ":" port ]
//*               "/" *( segment-nz "/" )
named!(serviceRoot<&str, &str>, recognize!(tuple!(alt!(tag_no_case!("https") | tag_no_case!("http")),
...
named!(odataRelativeUri<&str, &str>, alt!(recognize!(tuple!(tag!("$batch"), opt!(tuple!(tag!("?"), batchOptions))))
...
named!(resourcePath<&str, &str>, alt!(recognize!(tuple!(entitySetName, opt!(collectionNavigation)))
...
//*                      / count
//*                      / ref
named!(collectionNavPath<&str, &str>, alt!(recognize!(tuple!(keyPredicate, opt!(singleNavigation)))
...
named!(keyPredicate<&str, &str>, alt!(simpleKey | compoundKey | keyPathSegments));
//* simpleKey        = OPEN ( parameterAlias / keyPropertyValue ) CLOSE
named!(simpleKey<&str, &str>, recognize!(tuple!(OPEN, alt!(parameterAlias | keyPropertyValue), CLOSE)));
...
named!(keyValuePair<&str, &str>, recognize!(tuple!(alt!(primitiveKeyProperty | keyPropertyAlias), EQ, alt!(parameterAlias | keyPropertyValue))));
...
//*                    ]
named!(singleNavigation<&str, &str>, recognize!(tuple!(opt!(tuple!(tag!("/"), qualifiedEntityTypeName)), opt!(alt!(recognize!(tuple!(tag!("/"), propertyPath))
														   | boundOperation
														   | _ref
														   | value
														   )))));
...
named!(propertyPath<&str, &str>, recognize!(alt!(tuple!(entityColNavigationProperty, opt!(collectionNavigation))
...
named!(primitiveColPath<&str, &str>, alt!(count | boundOperation | ordinalIndex));
//*
//* primitivePath  = value / boundOperation
named!(primitivePath<&str, &str>, alt!(value | boundOperation));
//*
//* complexColPath = ordinalIndex
//*                / [ "/" qualifiedComplexTypeName ] [ count / boundOperation ]
named!(complexColPath<&str, &str>, alt!(ordinalIndex | recognize!(tuple!(opt!(tuple!(tag!("/"), qualifiedComplexTypeName)), opt!(alt!(count | boundOperation))))));
//*
//* complexPath    = [ "/" qualifiedComplexTypeName ]
//*                  [ "/" propertyPath
//*                  / boundOperation
//*                  ]
named!(complexPath<&str, &str>, recognize!(tuple!(opt!(tuple!(tag!("/"), qualifiedComplexTypeName)), opt!(alt!(recognize!(tuple!(tag!("/"), propertyPath)) | boundOperation)))));
//*
...
//*                      )
named!(boundOperation<&str, &str>, recognize!(tuple!(tag!("/"), alt!(boundActionCall
...
named!(boundFunctionCallNoParens<&str, &str>, recognize!(alt!(tuple!(namespace, tag!("."), entityFunction)
...
named!(functionImportCallNoParens<&str, &str>, alt!(entityFunctionImport
						    | entityColFunctionImport
						    | complexFunctionImport
						    | complexColFunctionImport
						    | primitiveFunctionImport
...
named!(functionParameter<&str, &str>, recognize!(tuple!(parameterName, EQ, alt!(parameterAlias | primitiveLiteral))));
...
//*              / nameAndValue
//*              / customQueryOption
named!(queryOption<&str, &str>, alt!(systemQueryOption | aliasAndValue | nameAndValue | customQueryOption));
//*
//* batchOptions = batchOption *( "&" batchOption )
named!(batchOptions<&str, &str>, recognize!(tuple!(batchOption, many0!(tuple!(tag!("&"), batchOption)))));
//* batchOption  = format
//*              /customQueryOption
named!(batchOption<&str, &str>, alt!(format | customQueryOption));
//*
...
//*                 /customQueryOption
named!(metadataOption<&str, &str>, alt!(format | customQueryOption));
//*
...
//*                / customQueryOption
named!(entityIdOption<&str, &str>, alt!(format | customQueryOption));
...
//*                   / expand
//*                   / select
named!(entityCastOption<&str, &str>, alt!(entityIdOption | expand | select));
//*
//* id = ( "$id" / "id" ) EQ IRI-in-query
named!(id<&str, &str>, recognize!(tuple!(alt!(tag_no_case!("$id") | tag_no_case!("id")), EQ, IRI_in_query)));
//*
...
//*                   / top
//*                   / index
named!(systemQueryOption<&str, &str>, alt!(compute
					   | deltatoken
					   | expand
					   | filter
					   | format
...
named!(compute<&str, &str>, recognize!(tuple!(alt!(tag_no_case!("$compute") | tag_no_case!("compute")), EQ, computeItem, many0!(tuple!(COMMA, computeItem)))));
...
named!(expand<&str, &str>, recognize!(tuple!(alt!(tag_no_case!("$expand") | tag_no_case!("expand")), EQ, expandItem, many0!(tuple!(COMMA, expandItem)))));
...
//*                     ]
named!(expandItem<&str, &str>, recognize!(alt!(recognize!(tuple!(STAR, opt!(alt!(_ref | recognize!(tuple!(OPEN, levels, CLOSE))))))
					       | tag_no_case!("$value")
...
//*                   / search
named!(expandCountOption<&str, &str>, alt!(filter | search));
//* expandRefOption   = expandCountOption
//*                   / orderby
//*                   / skip
//*                   / top
//*                   / inlinecount
named!(expandRefOption<&str, &str>, alt!(expandCountOption | orderby | skip | top | inlinecount));
...
//*                   / levels
//*                   / aliasAndValue
named!(expandOption<&str, &str>, alt!(expandRefOption | select | expand | compute | levels | aliasAndValue));
//*
//* levels = ( "$levels" / "levels" ) EQ ( oneToNine *DIGIT / "max" )
named!(levels<&str, &str>, recognize!(tuple!(alt!(tag_no_case!("$levels") | tag_no_case!("levels")), EQ, alt!(recognize!(tuple!(oneToNine, many0!(DIGIT))) | tag_no_case!("max")))));
//*
//* filter = ( "$filter" / "filter" ) EQ boolCommonExpr
named!(filter<&str, &str>, recognize!(tuple!(alt!(tag_no_case!("$filter") | tag_no_case!("filter")), EQ, boolCommonExpr)));
//*
//* orderby     = ( "$orderby" / "orderby" ) EQ orderbyItem *( COMMA orderbyItem )
named!(orderby<&str, &str>, recognize!(tuple!(alt!(tag_no_case!("$orderby") | tag_no_case!("orderby")), EQ, orderbyItem, many0!(tuple!(COMMA, orderbyItem)))));
//* orderbyItem = commonExpr [ RWS ( "asc" / "desc" ) ]
named!(orderbyItem<&str, &str>, recognize!(tuple!(commonExpr, opt!(tuple!(RWS, alt!(tag_no_case!("asc") | tag_no_case!("desc")))))));
//*
//* skip = ( "$skip" / "skip" ) EQ 1*DIGIT
named!(skip<&str, &str>, recognize!(tuple!(alt!(tag_no_case!("$skip") | tag_no_case!("skip")), EQ, many1!(DIGIT))));
//* top  = ( "$top"  / "top"  ) EQ 1*DIGIT
named!(top<&str, &str>, recognize!(tuple!(alt!(tag_no_case!("$top") | tag_no_case!("top")), EQ, many1!(DIGIT))));
//*
//* index  = ( "$index" / "index" ) EQ 1*DIGIT
named!(index<&str, &str>, recognize!(tuple!(alt!(tag_no_case!("$index") | tag_no_case!("index")), EQ, many1!(DIGIT))));
//*
...
named!(inlinecount<&str, &str>, recognize!(tuple!(alt!(tag_no_case!("$count") | tag_no_case!("count")), EQ, booleanValue)));
//*
//* schemaversion   = ( "$schemaversion" / "schemaversion" ) EQ ( STAR / 1*unreserved )
named!(schemaversion<&str, &str>, recognize!(tuple!(alt!(tag_no_case!("$schemaversion") | tag_no_case!("schemaversion")), EQ, alt!(STAR | recognize!(many1!(unreserved))))));
//*
//* search     = ( "$search" / "search" ) EQ BWS searchExpr
named!(search<&str, &str>, recognize!(tuple!(alt!(tag_no_case!("$search") | tag_no_case!("search")), EQ, BWS, searchExpr)));
//* searchExpr = ( OPEN BWS searchExpr BWS CLOSE
//*              / searchTerm
//*              ) [ searchOrExpr
//*                / searchAndExpr
//*                ]
named!(searchExpr<&str, &str>, recognize!(tuple!(alt!(recognize!(tuple!(OPEN, BWS, searchExpr, BWS, CLOSE)) | searchTerm), opt!(alt!(searchOrExpr | searchAndExpr)))));
//*
...
named!(searchTerm<&str, &str>, recognize!(tuple!(opt!(tuple!(tag!("NOT"), RWS)), alt!(searchPhrase | searchWord))));
...
named!(searchWord<&str, &str>, recognize!(many1!(alt!(ALPHA | DIGIT | COMMA | tag!(".") | pct_encoded))));
//*
//* select         = ( "$select" / "select" ) EQ selectItem *( COMMA selectItem )
named!(select<&str, &str>, recognize!(tuple!(alt!(tag_no_case!("$select") | tag_no_case!("select")), EQ, selectItem, many0!(tuple!(COMMA, selectItem)))));
//* selectItem     = STAR
...
//*                  )
named!(selectItem<&str, &str>, alt!(STAR
				    | allOperationsInSchema
...
//*                             / "/" selectProperty
//*                             ]
named!(selectProperty<&str, &str>, alt!(primitiveProperty
...
named!(selectPath<&str, &str>, recognize!(tuple!(alt!(complexProperty | complexColProperty), opt!(tuple!(tag!("/"), qualifiedComplexTypeName)))));
//* selectOptionPC = filter / search / inlinecount / orderby / skip / top
named!(selectOptionPC<&str, &str>, alt!(filter | search | inlinecount | orderby | skip | top));
//* selectOption   = selectOptionPC
//*                / compute / select / expand / aliasAndValue
named!(selectOption<&str, &str>, alt!(selectOptionPC | compute | select | expand | aliasAndValue));
//*
...
//*                / commonExpr
named!(parameterValue<&str, &str>, alt!(arrayOrObject | commonExpr));
//*
...
named!(contextFragment<&str, &str>, alt!(tag!("Collection($ref)")
					 | tag!("$ref")
...
//*                      / selectListProperty
//*                      )
named!(selectListItem<&str, &str>, alt!(STAR
					| allOperationsInSchema
...
named!(selectListProperty<&str, &str>, alt!(primitiveProperty
					    | primitiveColProperty
...
named!(contextPropertyPath<&str, &str>, alt!(primitiveProperty
					     | primitiveColProperty
					     | complexColProperty
...
named!(rootExpr<&str, &str>, recognize!(tuple!(tag!("$root/"), alt!(recognize!(tuple!(entitySetName, keyPredicate)) | singletonEntity), opt!(singleNavigationExpr))));
//*
//* firstMemberExpr = memberExpr
//*                 / inscopeVariableExpr [ "/" memberExpr ]
named!(firstMemberExpr<&str, &str>, alt!(memberExpr | recognize!(tuple!(inscopeVariableExpr, opt!(tuple!(tag!("/"), memberExpr))))));
//*
...
//*              / annotationExpr
//*              )
named!(memberExpr<&str, &str>, recognize!(tuple!(opt!(tuple!(qualifiedEntityTypeName, tag!("/"))), alt!(propertyPathExpr | boundFunctionExpr | annotationExpr))));
//*
...
//*                    )
named!(propertyPathExpr<&str, &str>, recognize!(alt!(
...
//*                  / primitivePathExpr
//*                  ]
named!(annotationExpr<&str, &str>, recognize!(tuple!(annotation, opt!(alt!(collectionPathExpr | singleNavigationExpr | complexPathExpr | primitivePathExpr)))));
//*
...
//TODO(validation)
named!(inscopeVariableExpr<&str, &str>, alt!(implicitVariableExpr | parameterAlias | lambdaVariableExpr));
...
named!(implicitVariableExpr<&str, &str>, alt!(tag!("$it") | tag!("$this")));
...
//*                            / collectionPathExpr
//*                            ]
named!(collectionNavigationExpr<&str, &str>, recognize!(tuple!(opt!(tuple!(tag!("/"), qualifiedEntityTypeName)), opt!(alt!(
...
//*                    / "/" anyExpr
//*                    / "/" allExpr
named!(collectionPathExpr<&str, &str>, alt!(
					    recognize!(tuple!(count, opt!(tuple!(OPEN, expandCountOption, many0!(tuple!(SEMI, expandCountOption)), CLOSE))))
...
named!(primitivePathExpr<&str, &str>, recognize!(tuple!(tag!("/"), opt!(alt!(annotationExpr | boundFunctionExpr)))));
//*
...
named!(functionExprParameter<&str, &str>, recognize!(tuple!(parameterName, EQ, alt!(parameterAlias | parameterValue))));
//*
...
//*                / nowMethodCallExpr
//*                / boolMethodCallExpr
named!(methodCallExpr<&str, &str>, alt!(indexOfMethodCallExpr
					| toLowerMethodCallExpr
					| toUpperMethodCallExpr
					| trimMethodCallExpr
					| substringMethodCallExpr
...
named!(boolMethodCallExpr<&str, &str>, alt!(endsWithMethodCallExpr
					    | startsWithMethodCallExpr
					    | containsMethodCallExpr
					    | intersectsMethodCallExpr
					    | hasSubsetMethodCallExpr
...
//*               / rootExprCol
//*               / primitiveColInUri
named!(arrayOrObject<&str, &str>, alt!(complexColInUri | complexInUri | rootExprCol | primitiveColInUri));
//*
...
//*                             complexColInUri
//*                           )
named!(collectionPropertyInUri<&str, &str>, alt!(recognize!(tuple!(quotation_mark, primitiveColProperty, quotation_mark, name_separator, primitiveColInUri))
...
named!(navigationPropertyInUri<&str, &str>, alt!(singleNavPropInJSON | collectionNavPropInJSON));
...
named!(begin_object<&str, &str>, recognize!(tuple!(BWS, alt!(tag!("{") | tag!("%7B")))));
//* end-object   = BWS ( "}" / "%7D" )
named!(end_object<&str, &str>, recognize!(tuple!(BWS, alt!(tag!("}") | tag!("%7D")))));
//*
//* begin-array = BWS ( "[" / "%5B" ) BWS
named!(begin_array<&str, &str>, recognize!(tuple!(BWS, alt!(tag!("[") | tag!("%5B")))));
//* end-array   = BWS ( "]" / "%5D" )
named!(end_array<&str, &str>, recognize!(tuple!(BWS, alt!(tag!("]") | tag!("%5D")))));
//*
//* quotation-mark  = DQUOTE / "%22"
named!(quotation_mark<&str, &str>, alt!(recognize!(DQUOTE) | tag!("%22")));
...
//*                        / 'false'
//*                        / 'null'
named!(primitiveLiteralInJSON<&str, &str>, alt!(stringInJSON | numberInJSON | tag!("true") | tag!("false") | tag!("null")));
//*
...
//*                       / 'u' 4HEXDIG     ;                 U+XXXX
//*                       )
named!(charInJSON<&str, &str>, alt!(qchar_unescaped | qchar_JSON_special | recognize!(tuple!(escape, alt!(  quotation_mark
													  | escape
													  | alt!(tag!("/") | tag!("%2F"))
...
named!(qchar_JSON_special<&str, &str>, alt!(SP | recognize!(one_of!(":{}[]"))));
//*
//* escape = "\" / "%5C"     ; reverse solidus U+005C
named!(escape<&str, &str>, alt!(tag!("\\") | tag!("%5C")));
//*
//* numberInJSON = [ "-" ] int [ frac ] [ exp ]
named!(numberInJSON<&str, &str>, recognize!(tuple!(opt!(tag!("-")), int, opt!(frac), opt!(exp))));
//* int          = "0" / ( oneToNine *DIGIT )
named!(int<&str, &str>, alt!(tag!("0") | recognize!(tuple!(oneToNine, many0!(DIGIT)))));
//* frac         = "." 1*DIGIT
named!(frac<&str, &str>, recognize!(tuple!(tag!("."), many1!(DIGIT))));
//* exp          = "e" [ "-" / "+" ] 1*DIGIT
named!(exp<&str, &str>, recognize!(tuple!(tag_no_case!("e"), opt!(alt!(tag!("-") | tag!("+"))), many1!(DIGIT))));
//*
//*
//* ;------------------------------------------------------------------------------
//* ; 6. Names and identifiers
...
//*                         / primitiveTypeName
named!(singleQualifiedTypeName<&str, &str>, alt!(qualifiedEntityTypeName
						 | qualifiedComplexTypeName
						 | qualifiedTypeDefinitionName
						 | qualifiedEnumTypeName
						 | primitiveTypeName));
...
named!(qualifiedTypeName<&str, &str>, alt!(singleQualifiedTypeName
...
named!(identifierLeadingCharacter<&str, &str>, alt!(ALPHA | tag!("_")));
//* identifierCharacter         = ALPHA / "_" / DIGIT ; plus Unicode characters from the categories L, Nl, Nd, Mn, Mc, Pc, or Cf
named!(identifierCharacter<&str, &str>, alt!(ALPHA | tag!("_") | DIGIT));
//*
...
//*                            )
named!(primitiveTypeName<&str, &str>, recognize!(tuple!(tag!("Edm."), alt!(  tag!("Binary")
									   | tag!("Boolean")
									   | tag!("Byte")
									   | tag!("Date")
									   | tag!("DateTimeOffset")
...
//*                         / 'Geometry'
named!(abstractSpatialTypeName<&str, &str>, alt!(tag!("Geography") | tag!("Geometry")));
...
//*                         / 'Point'
//*                         / 'Polygon'
named!(concreteSpatialTypeName<&str, &str>, alt!(  tag!("Collection")
						 | tag!("LineString")
						 | tag!("MultiLineString")
						 | tag!("MultiPoint")
						 | tag!("MultiPolygon")
...
named!(primitiveProperty<&str, &str>, alt!(primitiveKeyProperty | primitiveNonKeyProperty));
...
named!(navigationProperty<&str, &str>, alt!(entityNavigationProperty | entityColNavigationProperty));
...
named!(function<&str, &str>, alt!( entityFunction
				 | entityColFunction
				 | complexFunction
				 | complexColFunction
				 | primitiveFunction
...
//*                  / geometryPoint
//*                  / geometryPolygon
named!(primitiveLiteral<&str, &str>, alt!(  nullValue
					  | booleanValue
					  | guidValue
					  | dateValue
					  | dateTimeOffsetValue
...
named!(primitiveValue<&str, &str>, alt!( booleanValue
				       | guidValue
				       | durationValue
				       | dateValue
				       | dateTimeOffsetValue
...
named!(binaryValue<&str, &str>, recognize!(tuple!(many0!(many_m_n!(4, 4, base64char)), opt!(alt!(base64b16 | base64b8)))));
...
named!(base64char<&str, &str>, alt!(ALPHA | DIGIT | tag!("-") | tag!("_")));
//*
//* booleanValue = "true" / "false"
named!(booleanValue<&str, &str>, alt!(tag_no_case!("true") | tag_no_case!("false")));
//*
//* decimalValue = [ SIGN ] 1*DIGIT [ "." 1*DIGIT ] [ "e" [ SIGN ] 1*DIGIT ] / nanInfinity
named!(decimalValue<&str, &str>, alt!(recognize!(tuple!(opt!(SIGN),
							many1!(DIGIT),
...
named!(nanInfinity<&str, &str>, alt!(tag!("NaN") | tag!("-INF") | tag!("INF")));
//*
...
named!(string<&str, &str>, recognize!(tuple!(SQUOTE, many0!(alt!(recognize!(SQUOTE_in_string) | recognize!(pchar_no_SQUOTE))), SQUOTE)));
...
named!(year<&str, &str>, recognize!(tuple!(opt!(tag!("-")), alt!(recognize!(tuple!(tag!("0"), many_m_n!(3, 3, DIGIT))) | recognize!(tuple!(oneToNine, many_m_n!(3, 3, DIGIT)))))));
//* month = "0" oneToNine
//*       / "1" ( "0" / "1" / "2" )
named!(month<&str, &str>, alt!(  recognize!(tuple!(tag!("0"), oneToNine))
			       | recognize!(tuple!(tag!("1"), one_of!("012")))
			       ));
//* day   = "0" oneToNine
//*       / ( "1" / "2" ) DIGIT
//*       / "3" ( "0" / "1" )
named!(day<&str, &str>, alt!(  recognize!(tuple!(tag!("0"), oneToNine))
			     | recognize!(tuple!(one_of!("12"), DIGIT))
			     | recognize!(tuple!(tag!("3"), one_of!("01")))
			     ));
//* hour   = ( "0" / "1" ) DIGIT
//*        / "2" ( "0" / "1" / "2" / "3" )
named!(hour<&str, &str>, alt!(  recognize!(tuple!(one_of!("01"), DIGIT))
...
named!(singleEnumValue<&str, &str>, alt!(enumerationMember | enumMemberValue));
...
//*                       / pointLiteral
//*                       / polygonLiteral
named!(geoLiteral<&str, &str>, alt!(  collectionLiteral
				    | lineStringLiteral
				    | multiPointLiteral
				    | multiLineStringLiteral
				    | multiPolygonLiteral
...
named!(OWS<&str, &str>, recognize!(many0!(alt!(SP | HTAB))));
//* BWS-h = *( SP / HTAB )  ; "bad" whitespace in header values
named!(BWS_h<&str, &str>, recognize!(many0!(alt!(SP | HTAB))));
//* EQ-h  = BWS-h EQ BWS-h
...
named!(RWS<&str, &str>, recognize!(many1!(alt!(SP | HTAB | tag!("%20") | tag!("%09")))));
//* BWS =  *( SP / HTAB / "%20" / "%09" )  ; "bad" whitespace
named!(BWS<&str, &str>, recognize!(many0!(alt!(SP | HTAB | tag!("%20") | tag!("%09")))));
//*
//* AT     = "@" / "%40"
named!(AT<&str, &str>, alt!(tag!("@") | tag!("%40")));
//* COLON  = ":" / "%3A"
named!(COLON<&str, &str>, alt!(tag!(":") | tag!("%3A")));
//* COMMA  = "," / "%2C"
named!(COMMA<&str, &str>, alt!(tag!(",") | tag!("%2C")));
//* EQ     = "="
named!(EQ<&str, &str>, tag!("="));
//* SIGN   = "+" / "%2B" / "-"
named!(SIGN<&str, &str>, alt!(tag!("+") | tag!("%3B") | tag!("-")));
//* SEMI   = ";" / "%3B"
named!(SEMI<&str, &str>, alt!(tag!(";") | tag!("%3B")));
//* STAR   = "*" / "%2A"
named!(STAR<&str, &str>, alt!(tag!("*") | tag!("%2A")));
//* SQUOTE = "'" / "%27"
named!(SQUOTE<&str, &str>, alt!(tag!("'") | tag!("%27")));
//*
//* OPEN  = "(" / "%28"
named!(OPEN<&str, &str>, alt!(tag!("(") | tag!("%28")));
//* CLOSE = ")" / "%29"
named!(CLOSE<&str, &str>, alt!(tag!(")") | tag!("%29")));
//*
//*
//* ;------------------------------------------------------------------------------
//* ; A. URI syntax [RFC3986]
...
//*               / path-rootless
//* ;              / path-empty
named!(hier_part<&str, &str>, recognize!(tuple!(tag!("//"), authority, alt!(path_abempty | path_absolute | path_rootless | path_empty))));
...
named!(scheme<&str, &str>, recognize!(tuple!(ALPHA, many0!(alt!(ALPHA | DIGIT | tag!("+") | tag!("-") | tag!("."))))));
//* authority     = [ userinfo "@" ] host [ ":" port ]
named!(authority<&str, &str>, recognize!(tuple!(opt!(tuple!(userinfo, tag!("@"))), host, opt!(tuple!(tag!(":"), port)))));
//* userinfo      = *( unreserved / pct-encoded / sub-delims / ":" )
named!(userinfo<&str, &str>, recognize!(many0!(alt!(unreserved | pct_encoded | sub_delims | tag!(":")))));
//* host          = IP-literal / IPv4address / reg-name
named!(host<&str, &str>, alt!(IP_literal | IPv4address | reg_name));
//* port          = *DIGIT
named!(port<&str, &str>, recognize!(many0!(DIGIT)));
//* IP-literal    = "[" ( IPv6address / IPvFuture  ) "]"
named!(IP_literal<&str, &str>, delimited!(tag!("["), alt!(IPv6address | IPvFuture), tag!("]")));
//* IPvFuture     = "v" 1*HEXDIG "." 1*( unreserved / sub-delims / ":" )
named!(IPvFuture<&str, &str>, recognize!(tuple!(tag!("v"), many1!(HEXDIG), tag!("."), many1!(alt!(unreserved | sub_delims | tag!(":"))))));
...
named!(ls32<&str, &str>, alt!(recognize!(separated_pair!(h16, tag!(":"), h16)) | IPv4address));
...
named!(reg_name<&str, &str>, recognize!(many0!(alt!(unreserved | pct_encoded | sub_delims))));
...
named!(pchar<&str, &str>, alt!(unreserved | pct_encoded | sub_delims | recognize!(one_of!(":@"))));
//* query         = *( pchar / "/" / "?" )
named!(query<&str, &str>, recognize!(many0!(alt!(pchar | recognize!(one_of!("/?"))))));
//* fragment      = *( pchar / "/" / "?" )
named!(fragment<&str, &str>, recognize!(many0!(alt!(pchar | recognize!(one_of!("/?"))))));
//* pct-encoded   = "%" HEXDIG HEXDIG
named!(pct_encoded<&str, &str>, recognize!(do_parse!(tag!("%") >> HEXDIG >> HEXDIG >> ())));
//* unreserved    = ALPHA / DIGIT / "-" / "." / "_" / "~"
named!(unreserved<&str, &str>, alt!(ALPHA | DIGIT | recognize!(one_of!("-._~"))));
//* ;reserved      = gen-delims / sub-delims
//* ;gen-delims    = ":" / "/" / "?" / "#" / "[" / "]" / "@"
//* ;sub-delims    = "!" / "$" / "&" / "'" / "(" / ")" / "*" / "+" / "," / ";" / "="
//* sub-delims     =       "$" / "&" / "'" /                                     "=" / other-delims
named!(sub_delims<&str, &str>, alt!(recognize!(one_of!("$&'=")) | other_delims));
//* other-delims   = "!" /                   "(" / ")" / "*" / "+" / "," / ";"
named!(other_delims<&str, &str>, recognize!(one_of!("!()*+,;")));
//*
//* pchar-no-SQUOTE       = unreserved / pct-encoded-no-SQUOTE / other-delims / "$" / "&" / "=" / ":" / "@"
named!(pchar_no_SQUOTE<&str, &str>, alt!(unreserved | pct_encoded_no_SQUOTE | other_delims | recognize!(one_of!("$&=:@"))));
//* pct-encoded-no-SQUOTE = "%" ( "0" / "1" /   "3" / "4" / "5" / "6" / "8" / "9" / A-to-F ) HEXDIG
//*                       / "%" "2" ( "0" / "1" / "2" / "3" / "4" / "5" / "6" /   "8" / "9" / A-to-F )
named!(pct_encoded_no_SQUOTE<&str, &str>, alt!(  recognize!(tuple!(tag!("%"), one_of!("013456789ABCDEFabcdef"), HEXDIG))
					       | recognize!(tuple!(tag!("%2"), one_of!("012345689ABCDEFabcdef")))
					      ));
//*
//* qchar-no-AMP              = unreserved / pct-encoded / other-delims / ":" / "@" / "/" / "?" / "$" / "'" / "="
named!(qchar_no_AMP<&str, &str>, alt!(qchar_no_AMP_EQ_AT_DOLLAR | tag!("=")));
//* qchar-no-AMP-EQ           = unreserved / pct-encoded / other-delims / ":" / "@" / "/" / "?" / "$" / "'"
named!(qchar_no_AMP_EQ<&str, &str>, alt!(qchar_no_AMP_EQ_AT_DOLLAR | tag!("@") | tag!("$")));
//* qchar-no-AMP-EQ-AT-DOLLAR = unreserved / pct-encoded / other-delims / ":" /       "/" / "?" /       "'"
named!(qchar_no_AMP_EQ_AT_DOLLAR<&str, &str>, alt!(unreserved | pct_encoded | other_delims | recognize!(one_of!(":/?'"))));
//*
//* qchar-unescaped       = unreserved / pct-encoded-unescaped / other-delims / ":" / "@" / "/" / "?" / "$" / "'" / "="
named!(qchar_unescaped<&str, &str>, alt!(unreserved | pct_encoded_unscaped | other_delims | recognize!(one_of!(":@/?$'="))));
//* pct-encoded-unescaped = "%" ( "0" / "1" /   "3" / "4" /   "6" / "7" / "8" / "9" / A-to-F ) HEXDIG
//*                       / "%" "2" ( "0" / "1" /   "3" / "4" / "5" / "6" / "7" / "8" / "9" / A-to-F )
//*                       / "%" "5" ( DIGIT / "A" / "B" /   "D" / "E" / "F" )
named!(pct_encoded_unscaped<&str, &str>, alt!( recognize!(tuple!(tag!("%"), alt!(recognize!(one_of!("01346789")) | A_to_F), HEXDIG))
...
named!(qchar_no_AMP_DQUOTE<&str, &str>, alt!(qchar_unescaped | recognize!(tuple!(escape, alt!(escape | quotation_mark)))));
//*
//*
//* ;------------------------------------------------------------------------------
...
named!(IRI_in_header<&str, &str>, recognize!(many1!(alt!(VCHAR | obs_text))));
...
fn main() {
	println!("{:?}", odataRelativeUri("ProductsByComplex(complex=@c)?@c={\"@odata.type\":\"Model.Customer\",\"Name\":\"Value\"}\n"));
}
